...
##import the data (100 runs for each file)
votes_1 <- read.delim("2018-09-05-vote.tsv",sep="\t")
votes_2 <- read.delim("2018-10-19-vote.tsv",sep="\t")
votes_3 <- read.delim("2018-10-23-vote.tsv",sep="\t")
votes_4 <- read.delim("2018-10-25-vote.tsv",sep="\t")
votes_5 <- read.delim("2018-10-27-vote.tsv",sep="\t")
votes_preremoved <- rbind(votes_1,votes_2,votes_3,votes_4,votes_5)

### data generated by updated files
##update the retrieve-recognition data 
votes_removed1 <- filter(votes_preremoved, model != "Random_Retrieve-Recognize-Party_VG" & model != "Serial_Retrieve-Recognize-Party_VG")
votes_6 <- read.delim("2019-02-15-vote.tsv",sep="\t")
#keep the model names consistant
votes_6 <- data.frame(lapply(votes_6, function(x) {gsub("VG-Serial-Retrieve-Recognize-Party", "Serial_Retrieve-Recognize-Party_VG", x)}))
votes_6 <- data.frame(lapply(votes_6, function(x) {gsub("VG-Random-Retrieve-Recognize-Party", "Random_Retrieve-Recognize-Party_VG", x)}))

v1 <- rbind(votes_removed1,votes_6)

##update the party data 
votes_removed2 <- filter(v1, model != "Serial_Party_VG" & model != "Random_Party_VG")
votes_7 <- read.delim("2019-03-07-vote.tsv",sep="\t")
votes_7 <- data.frame(lapply(votes_7, function(x) {gsub("VG-Random-RetrieveParty-Party", "Random_Party_VG", x)}))
votes_7 <- data.frame(lapply(votes_7, function(x) {gsub("VG-Serial-RetrieveParty-Party", "Serial_Party_VG", x)}))

v2 <- rbind(votes_removed2,votes_7)

##update the random recognition data 
votes_removed3 <- filter(v2, model != "Random_Retrieve-Recognize-Party_VG" & model != "Random_Recognize-Party_VG")
votes_8 <- read.delim("2019-03-11-vote.tsv",sep="\t")
votes_8 <- data.frame(lapply(votes_8, function(x) {gsub("VG-Random-Recognize-Party", "Random_Recognize-Party_VG", x)}))
votes_8 <- data.frame(lapply(votes_8, function(x) {gsub("VG-Random-Retrieve-Recognize-Party", "Random_Retrieve-Recognize-Party_VG", x)}))

votes <- rbind(votes_removed3,votes_8)
